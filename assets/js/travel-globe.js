(async function () {
  ...
  // ✅ CSP(unsafe-eval) 피하려고 skypack 대신 "순수 ESM 파일"을 dynamic import로 로드
  let createGlobe;
  try {
    const mod = await import("https://cdn.jsdelivr.net/npm/cobe@0.6.5/dist/index.esm.js");
    createGlobe = mod.default;
  } catch (e) {
    console.error("[travel-globe] COBE import failed:", e);
    if (elStats) elStats.textContent = `Globe failed to load (CSP/CDN). Console 확인해줘.`;
    return;
  }
  ...
  // ====== 너 데이터(네가 준 내용 + 네가 추가한 미국 로드트립 일부) ======
  const places = [
    // --- Asia ---
