---
layout: single
title: "Publications & Presentations"
permalink: /publications/
author_profile: true
toc: true
---

## 1. Publications

### 📄 Papers

* <div style="display: flex; justify-content: space-between; align-items: flex-start;">
    <div style="flex: 1; padding-right: 20px;">
      **Design, Fabrication, and Testing of a Reusable Launch Vehicle Prototype System**
      <br>
      Kwon, M.Y., Jeong, W.J., ..., **Ha, S.H.** (2023).
      <br>
      *Capstone Design Project, Korea Aerospace University.*
      <br>
      Conducted roll control experiments and validated attitude controllers for a reusable rocket prototype.
    </div>
    <div style="flex: 0 0 300px;"> <img src="/assets/images/VFCV_2.jpg" alt="VFCV Project Image" style="width: 100%; height: auto; border-radius: 8px; box-shadow: 0 4px 8px rgba(0,0,0,0.1);">
    </div>
  </div>

* **A Study on the Temperature Control Performance of an HVAC System through Upper and Lower Power Distribution of a High-Voltage PTC Heater for Electric Vehicles**
    * Kim, H.U., ..., **Ha, S.H.**, and Kim, U.H. (2025).
    * *Presented at the HMG Technical Conference, 2025.*
    * Investigated energy efficiency optimization for next-generation EV thermal management systems.

---

### 💡 Patents (Selected)
*Lead Inventor on multiple HVAC and thermal management patents at Hyundai WIA.*

* **Air Distributor for Rear HVAC Unit** (10-2024-0197822, **Patent Pending**)
    * Innovated a single-door mode unit to reduce system package size and production cost.
* **Intake Apparatus of a Vehicle** (10-2025-0005531, **Patent Pending**)
    * Improved fresh/recirculated air linearity through an optimized internal partition wall.
* **Temperature Regulating Door for HVAC System** (10-2025-0005532, **Patent Pending**)
    * Enhanced refrigerant condensation efficiency by tuning the door opening sequence.
* **HVAC Apparatus Case** (10-2024-0116318, **Patent Pending**)
    * Designed a barrier structure to prevent foreign object buildup in the drain path.
* **Temperature Door of HVAC for Vehicle** (10-2024-0174599, **Patent Pending**)
* **Case for HVAC System** (10-2024-0178349, **Patent Pending**)
* **Blower of HVAC System for Vehicle** (10-2024-0180131, **Patent Pending**)

*Co-Inventor on 10+ additional patents in air distribution and EV thermal management.*

---

## 2. Presentations

* <div style="display: flex; justify-content: space-between; align-items: flex-start;">
    <div style="flex: 1; padding-right: 20px;">
      **Next-Generation HVAC Development Concepts**
      <br>
      *GM TECH DAY Seminar (Oct 2025, Detroit).*
      <br>
      Selected as a technical representative to present next-generation thermal solutions to global engineering teams.
    </div>
    <div style="flex: 0 0 300px;">
      <img src="/assets/images/GM_1.jpg" alt="GM Tech Day Presentation" style="width: 100%; height: auto; border-radius: 8px; box-shadow: 0 4px 8px rgba(0,0,0,0.1);">
    </div>
  </div>

* **Design and Testing of Reusable Launch Vehicle Prototype**
    * *Excellence Prize, Capstone Design Competition, Korea Aerospace University (2023).*
